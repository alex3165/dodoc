  ...
  return {
    ...
    importNewMedia : function() {
      ...
      var $modal = $('#modal-add-local');
      	...
      	$filePicker.on( 'change', function( e ) {
    			var filesData = e.originalEvent.target.files;
        if( filesData.length > 0) {
          var fileNames = $.map( filesData, function(val) {
              return val.name !== undefined ? val.name:'';
          });
          	$filePicker.data('filesData', filesData);
      			$label.html( fileNames.join('<br>'));
        } else {
          	$filePicker.data('filesData', '');
      			$label.html( labelVal);
      		}
      	});

      $modal.find('.js--modal_submit').on('click',function(){
        	var filesData = $filePicker.data( 'filesData');
        	//Images changed

        	if( filesData !== undefined){
        		console.log('Sending new medias to dodoc server.');
          ...
          for (var i = 0; i < filesData.length; i++) {
            ...
            formData.append('uploads[]', file, file.name);
      		  ...
      		  sendData.importMedias( formData);
